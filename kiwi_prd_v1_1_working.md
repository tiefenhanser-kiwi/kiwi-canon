<!-- ============================================================
     MIRROR COPY — generated 2026-09-02 23:43Z (UTC) by chat-Claude from Claude project knowledge.
     Source of truth is project knowledge. This file is a READ-ONLY snapshot.

     NOT AUTHORITATIVE FOR, EVER:
       - current block / current position / HEAD
       - any next-available ID counter (D-WS7, D-WS9, BUG)
     Those come from your PROMPT ONLY. A counter cached outside its log is
     stale by construction — this project has already spent an amend undoing
     a collision minted from a cached counter.

     AUTHORITATIVE ENOUGH TO READ FOR: rulings, rationale, bug history,
     design intent, specs, prior decisions. Grep it instead of asking.
     If a ruling here contradicts your prompt, SAY SO — do not silently pick one.
     ============================================================ -->

# Kiwi — Product Requirements Document — v1.1 WORKING

**STATUS: WORKING DOCUMENT — Active editing copy.** Tracks deltas from v1.0 locked. Initial state at creation: identical to v1.0.

**Created:** April 30, 2026 (immediately after v1.0 lock)
**Frozen reference:** `kiwi_prd_v1.0_locked.md` — do not edit; v1.0 is the baseline for all future deltas.
**Edit log:** When this document changes, note the change in the "Change log" section at the end.
**Authoritative scope:** Same as v1.0 — supersedes D1/D2/D3 per §1.9.

---


**Status:** Living document. Updated as decisions are made. Sections marked [LOCKED] are final unless explicitly reopened.
**Last updated:** July 13, 2026 (**WS7-8b CLOSE → WS7-8 CLOSE** — **six redlines applied, all in §13 (Cook Mode / post-cook)**, plus one net-new subsection. **(1) §13.5.4** — ordering **criterion 1 changed from "aligning finish times" to "nothing finishes materially early"** (under a single cook, identical finish times are often **unachievable** — a human serializes active steps, and only *unattended* windows permit overlap); the **criteria order is now flagged LOAD-BEARING**, since BUG-018 was an implementation optimizing criterion 2 *at the cost of* criterion 1. Its `[SCHEMA]` note redlined: **`parallelGroup` is RETIRED** (NULL on 1959/1959, now write-rejected) → the Sequencer reads **`phaseType` + `estimatedMinutes` + `isTimingSensitive`**. **(2) §13.5.5** — the *"runs deterministically… free"* clause is now **TRUE and VERIFIED**; it was **not true before** (the Sequencer had been a Sonnet call). Flagged as **load-bearing pricing, not description**: any future AI call here breaks the free-tier promise and must redline first. **(3) §13.6** — the "parallel grouping" translation bullet described the retired field; concurrency is now **derived at schedule time from `isTimingSensitive`**, not tagged at translation time. **(4) §13.5.7** — **completion-screen copy APPROVED** ("All done — enjoy a great meal by Chez [Name]!" / "How was it? Your rating tunes future plans."); **the two-buttons-vs-two-buttons contradiction with §13.10.1 RESOLVED to ONE button ("Done" → Meal Detail)** — built verbatim the spec produced **four buttons and two competing exits**; the `meal_cooked` event name corrected to **`cook_mode_completed`** (`meal_cooked` never existed in §13.7's list). **(5) §13.10.1/§13.10.2** — the calibration card has **no buttons of its own**; **fork-on-calibrate** flagged as the one path that can **corrupt shared catalog data** (never write a personal rating to a `userId: null` row — reuse D-WS7-139's fork-on-acquire). **(6) §13.10.3 `[SCHEMA]` — THIS NOTE WAS WRONG and was WS7-11-blocking:** it claimed *"No new fields required"* because it counted only `estimatedTimeMinutes`/`difficulty` — **it forgot the stars.** There is nowhere on `Meal` to store a rating today → **WS7-11 requires a migration.** **NEW §13.10.4 — Favorites, ratings, and cook-tracking:** three concepts that must not be conflated — **Saved** (the heart = save-to-library, *not* a rating) vs **Rated** (stars) vs **Favorite** (forward-looking intent). **Stars cannot subsume favorites — quality and reach-for-it-ness are uncorrelated** (a 5-star meal can be a once-a-year showpiece; a 3-star meal the kids eat on a Tuesday is exactly a favorite). Also documents that **`timesCooked`/`lastUsedAt` are dead columns (0/337)** — *nothing in the product currently knows a meal was cooked* — and rules that the counter **increments on DONE, not on Cook-tap**. **All §13 post-cook items are owned by the new WS7-11 workstream, sequenced after the WS9 restyle.** Prior note, kept for history: July 12, 2026, WS7-8b Block B2 close — **two redlines applied**: (1) **§2.8 `[OPEN]` → `[LOCKED]`** — partial-package display resolved in favor of the **two-part `purchase size (need quantity)` line**, *reversing* this section's prior "probably just show the pack" lean; the §2.8 conversion-table reference-data note also updated to **[BUILT]** with the **provenance-stamp invariant** (`curated` / `usda_derived` / `ai_estimated` — a wrong value wearing an authoritative stamp is worse than an honest miss). (2) **§13.4.2 auto-advance addendum** — BUG-024's queued redline from Block A, carried since July 9 and landed at this close.)

**Prior update:** July 4, 2026 (WS7-8b footer arc close — §13.4.2 redlined for the shipped Week Prep footer redesign + progress-bar partial state per BUG-020 / D-WS7-188, `0c5d60e`; §13.4.3 deliberately NOT redlined — the D-WS7-187 per-dish blend split is PRD-correcting, not PRD-diverging)
**Prior update:** May 17, 2026 (6c-7 close — §10.4.1 redlined to add server-side image resize backstop per Block 3 work; client + server now perform symmetric resize)
**Earlier update:** May 13, 2026 (6c-3 sub-workstream close — §10.4 Image Import redlined for shipped reality including locked-decision revision on resize parameters, §10.4b Text Import redlined for shipped 50-char min, §10.4.3 ambiguous-flag-banner clarified as deferred to WS9 per D-WS6-058)
**Prior update:** May 11, 2026 (6c-1 sub-workstream close — §10.2 entry points add Text Import sibling, §10.3.1 helper-text rewrite, §10.3.4 error UX redesign with Image+Text primary CTAs, new §10.4b Text Import section, §10.9 anti-bot landscape note added)
...
**[REDLINE PENDING WS7-CLOSE — WS7-7-A B1, June 11, 2026]** Schema additions to this LOCKED model, to be written in at WS7-CLOSE: (1) `isUserAdded` boolean — distinguishes plan-derived rows (reconciled on plan change) from user-typed Extras (never touched by reconcile); (2) the "source (which meal it came from)" is implemented as a **`GroceryListItemSource` join table** (`groceryListItemId, mealId, dishId`, cascade-delete), not singular columns, because a consolidated row legitimately has multiple source meals (e.g. garlic from three meals) — this multi-source provenance drives the WS7-7 incremental delta-reconcile (D-WS7-079). The earlier dormant singular `sourceMealId`/`sourceDishId` columns were dropped in favor of the join table.
...
**[BUILT — July 12, 2026, WS7-8b Block B2]** This table now exists: `ingredientConversions.ts` (source-of-truth code module, seeded onto `Ingredient.conversionRef Json?`). It is **one shared table with two consumers** — grocery purchase-sizing *and* the macro quantity→grams conversion (closing D-WS6-024 Step 2, which specified the same kitchen-density service). A grocery-local table would have guaranteed two divergent sources of density truth.

Row shape: `gramsPerCup` (volume↔weight; tbsp/tsp derive by fixed ratio) · `gramsPerEach` (count↔weight) · `subUnit{parent, perParent}` (head↔clove) · purchase fields · **`source`** · `confidence`.

**Coverage is complete across all 441 catalog ingredients; what varies is PROVENANCE, and every row is stamped:** `curated` (hand-authored — offenders, sub-units, count produce, grains) · `usda_derived` (from FDC `foodPortions`) · `ai_estimated` (runtime fallback on a table miss, self-populating).

**⚠️ The provenance stamp is load-bearing, not bookkeeping.** An AI fallback is a **guess**, not an authoritative answer. Writing a guess into the shared canonical catalog **unstamped** would make it indistinguishable from curated data and reuse it forever, for every user. **A wrong value wearing an authoritative stamp is strictly worse than an honest miss** — a miss falls back to an estimate that is at least *labelled* an estimate. Any future work extending this table must preserve the stamp.

**[LOCKED — resolved July 12, 2026, WS7-8b Block B2]** How to display partial-package ingredients on the user's list. **RESOLVED: show BOTH, as a single two-part line.**

**The grocery line is two-part: `purchase size (need quantity)`.**

> **1 wedge (6 oz) parmesan (4⅞ oz)**
> **3 heads garlic (30 cloves)**
> **5 lb bag flour (1½ cups)**
> **1 bottle cardamom (2 tbsp)**

- The **purchase size** is the rounded-to-a-real-world-purchasable pack (the covers-the-need number).
- The **need quantity** (in parentheses) is **decision-support data**. It exists so the user can judge *"do I actually need to buy this, or do I have enough in the cupboard?"* — ½ tsp cardamom, you probably have it, skip; 2 tbsp, buy the bottle. It must therefore stay **fine-grained** and **must NEVER be rounded toward a purchasable amount**. (Measured units round up on the **⅛** ladder — this section permits "nearest ⅛ **or** a clean fraction," and granularity matters for a decision-support number.)

**⚠️ This REVERSES this section's prior lean.** The original `[OPEN]` asked whether to show `"1 6oz can (you'll have leftover)"` or just `"1 6oz can"`, and guessed *"probably the latter."* **That guess was wrong, and the reasoning behind reversing it is the durable part:** the two numbers answer **different questions**. The pack answers *"what do I put in the cart?"*; the need answers *"do I need to cart anything at all?"* Collapsing to the pack alone silently discards the second question — the user can no longer tell a 2-tbsp need from a ½-tsp need, so they buy the jar of cardamom they already own. The parenthetical is not "leftover" framing; it is the decision input.

**Implementation invariant (load-bearing, do not regress):** the pack is **persisted as data** (`purchaseUnit`/`purchaseQuantity`/`purchaseDisplay` on `GroceryListItem`) and the need stays **data** (`quantityAmount`) — the line is **composed at render**. **Nothing formatted is ever persisted.** `quantityAmount` seeds the inline editor and is parsed on persist, so baking a formatted need into a display string would desync the editable value from what's shown. Fraction glyphs and count-noun pluralization are **render-only** formatting of authored values (`4⅞` is not the stored `4.875`).
...
**Trust signals shown:**
- Sub-button text: "14-day free trial · no credit card needed" ⚠️ **REDLINED August 11, 2026 (D-WS9-138): was "30-day". Hans ruled 14 days; the business plan already modeled 14. Shipped in `1b50303`.**
...
**Household size** — stepper. Default 4. Editable from 1 to 30. *[REDLINED 2026-05-21 / WS7-2-F: range widened from 1-12 to 1-30 to support one-off cases like Thanksgiving cooking for a large group. Implementation also adopted at 1-30. Label: "Household size" in onboarding (first-time-user clarity), "Default servings" in profile preferences (servings semantics). Possible WS9 polish to align both screens on one label.]*
...
**Wants leftovers** — toggle. Default OFF. *[REDLINED 2026-05-21 / WS7-2-F: added to step 2 per Hans's product direction (was not in original PRD §3.4 spec). Persists to UserPreferences.wantsLeftovers. Per D-WS7-035, wizard can override this per-plan-generation without writing back to UserPreferences.]*

**Plan length default** — chip row. Default 5. Editable from 1 to 7 (chips: 1, 2, 3, 4, 5, 6, 7). *[REDLINED 2026-05-21 / WS7-2-F: added to step 2 per Hans's product direction (was deferred to FUTURE per §6.12 pre-WS7-2-F; now in MVP). Range 1-7 (was 1-10 in §6.12 — range tightened because plans over 7 days introduce week-spanning complexities and users realistically don't prep for food more than a week out). Per D-WS7-035, wizard can override this per-plan-generation without writing back to UserPreferences.]*
...
- ~~User-configurable plan length preference (default 5 meals, range 1-10) — both as a profile setting and per-run override; logged here so all wizard variants pick it up consistently. For MVP, plan length stays hardcoded at 5 except for Scenario E "Make it 6" path.~~ *[REDLINED 2026-05-21 / WS7-2-F: lifted from FUTURE. Plan length default landed in MVP onboarding step 2 (default 5, range 1-7 — range tightened from 1-10 because plans over 7 days introduce week-spanning complexities). Per D-WS7-035 (open), wizard per-run override of plan length, household size, and wants leftovers will land in WS7-3+ wizard work without writing back to UserPreferences defaults.]*
...
**Item ordering (server-canonical) [REDLINE LOCKED May 27, 2026 — WS7-4-D c15 implementation]:**

The server returns plan items in a deterministic order on every read path that returns an items collection (GET `/plans/:id`, GET `/plans/templates/:id`):

1. **Assigned items first**, sorted by `assignedDayOfWeek` Sunday → Monday → Tuesday → Wednesday → Thursday → Friday → Saturday.
2. **Unscheduled items last** (rows where `assignedDayOfWeek IS NULL`), pinned to the bottom of the list.
3. **Tiebreaker** within the same day (and within the unscheduled cluster): `positionIndex ASC` — preserves explicit user reordering.

The mobile Plan Review screen, future web app, and any analytics consumer see the same canonical order. Moving a meal from Saturday to Sunday raises its row in the list; moving it from Monday to Friday lowers its row.

Implementation: shared helper `sortPlanItemsCanonical<T>(items: T[]): T[]` in `artifacts/api-server/src/lib/planItemSort.ts`. Day ordinals: Sunday=0..Saturday=6, unscheduled=7. Tiebreaker `positionIndex` ASC. Returns a new array (non-mutating).

**Plan date wire-shape (server-canonical) [REDLINE LOCKED May 27, 2026 — WS7-4-D c16 implementation]:**

Plan `startDate` and `endDate` cross the API boundary as calendar-date strings in `YYYY-MM-DD` form, symmetric on read and write. The mobile date editor emits `YYYY-MM-DD` (deliberately, to avoid `toISOString` timezone-shift bugs); the server accepts that on write (PATCH `/plans/:id`, POST `/plans/use-template/:id`) and returns the same `YYYY-MM-DD` form on every read path (GET `/plans/:id`, GET `/home` activeInstance, list/summary discovery payloads). Moment-in-time ISO 8601 is reserved for audit-log timestamps (activity-event metadata) and is NOT used on user-facing plan date fields — mixing the two is what produced the "Invalid Date" rendering bug resolved in c16. Implementation: `toYmd(date): string | null` helper in `artifacts/api-server/src/lib/planQueries.ts` (UTC component extraction; symmetric inverse of the UTC-midnight `new Date("YYYY-MM-DD")` write parse). Any future plan-level date field added to a read payload applies `toYmd`.
...
- Plan Review → Add Meals → Import URL / Import Image / Import Text / Create Manually (per §8.3.8)
- Plan Review → Change Meal → Import URL / Import Image / Import Text / Create Manually (per §8.4.2)
- Plan Review → Change Recipe → Import URL / Import Image / Import Text / Create Manually (per §8.4.3)
- My Meals tab → "+ Add Meal" → Import URL / Import Image / Import Text / Create Manually (per §9.3.1)
...
**Sibling ordering:** Import URL is listed first by convention but is no longer the recommended-default given the anti-bot landscape (§10.9). All three imports are equal siblings; users typically pick by what they have on hand — a link, a screenshot, or copied text.
...
- Sub: "Kiwi can try to parse a public recipe URL into ingredients and steps for you"
- Large URL input: "https://www.example.com/recipe/…"
- Helper text: "Works with many recipe sites. If a URL doesn't work, try Import from Image or Import from Text instead."
- Primary button: "Import Recipe" (sub-text: "Kiwi fetches and parses the ingredients and steps")

**Note:** As of May 2026, several major recipe sites (AllRecipes, Serious Eats, Simply Recipes, Food52, Budget Bytes, Smitten Kitchen) actively block server-side fetches via Cloudflare or other anti-bot infrastructure. URL Import is therefore best-effort: it works on personal blogs and sites that allow server-side access, but fails on most large publisher sites. Image Import (§10.4) and Text Import (§10.4b) are the primary paths for content from blocked sites. See §10.9 for full anti-bot landscape discussion.
...
**URL doesn't parse:** Show a helpful error. "Kiwi couldn't read this recipe — the site may block automated access. Try one of these instead:" with three buttons:
- **Import from Image** (primary) — "Take a screenshot of the recipe on your screen, or photograph it"
- **Import from Text** (primary) — "Copy and paste the recipe text directly"
- **Create Manually** (secondary) — for users who want to build the meal from scratch

Common reasons for parse failure: Cloudflare or anti-bot block (most major recipe sites), paywall, JavaScript-rendered content, non-recipe URL. Logged for analytics. Per §10.9, this is expected behavior for most major publisher sites; Image and Text imports are the primary paths.
...
**[REDLINE May 13, 2026 — shipped reality]** Updated to match shipped UX (Block D, 6c-2 close):

- Header: "Import from photo"
- Title: "Snap or pick recipe photos"
- Subtitle: "Kiwi can parse a recipe from up to 5 images."
- Source buttons (vertical):
  - Primary "Choose from Library" (prominent) — opens device photo library, multi-select up to 5 images per import
  - Secondary "Take Photo" — opens camera, one image per launch
  - Both disabled when 5 images selected
- Thumbnail strip of selected images with × badges to remove individual images
- Counter: "X of 5 images"
- Primary "Import Recipe" button — enabled when ≥1 image selected
- Helper text: "Works with screenshots from any site, photos of cookbooks, or recipe cards."

**Original (pre-shipped, pre-6c-2 close):**
...
**Implementation note:** Client resizes each image to 1568px longest-edge @ JPEG quality 0.7 before upload (Anthropic's documented vision recommendation; revised May 12, 2026 from initial 2048px/0.8 spec after transport-layer payload-size testing). **[REDLINE May 17, 2026 — 6c-7 Block 3]** Server also performs a defensive resize at the same parameters (1568px longest-edge / JPEG q70) on any image whose longest edge exceeds 1568px OR whose raw size exceeds 500 KB, before forwarding to Anthropic. Backstop ensures non-mobile callers (future web client, API integrations, automated callers) don't hit transport-layer failures on oversized uploads. Mobile-resized payloads pass through the server resize as a no-op (same target parameters). Server enforces 5 MB per image, 25 MB total. See §10.9 for cost / behavior notes.
...
**[REDLINE May 13, 2026]** The "Ambiguous flag banner" described below is **not implemented as of 6c-2 close**. AI does return caveats from the server (e.g. "Steps not visible in photo; suggested cooking steps generated based on ingredients"), but the UI doesn't surface them per D-WS6-046 (caveats dropped at UI boundary). Per-item AI confidence flags (the ⚠ marker on individual ingredients) are deferred to WS9 per D-WS6-058 — requires schema addition (`confidence` field per ingredient/step), prompt body update to emit confidence signals, and UI render. Original spec retained below as the target state for WS9.

Same shape as URL import, with one important addition — **ambiguous parsing flags** [TARGET STATE — WS9, D-WS6-058]:
...
### 10.4b Import from Text [LOCKED, May 11, 2026 — sibling import method added during 6c-1 close; REDLINED May 13, 2026 for shipped reality]

For recipes the user has access to in text form: NYT Cooking subscriber view (copy-paste from browser), Reddit r/recipes comments, blog-post text, cookbook OCR results from a third-party app, family-recipe transcriptions, etc. Especially valuable for paywalled sources where the URL Import path fails but the user has legitimate access to the recipe content.

#### 10.4b.1 Input screen

**[REDLINE May 13, 2026 — shipped reality]** Updated to match shipped UX (6c-3 close):

- Header: "Import from text"
- Title: "Paste your recipe"
- Subtitle: "Copy a recipe from anywhere and paste it below — Kiwi will parse it into ingredients and steps."
- Large multiline text input (10 lines minimum height, paste-friendly, `autoCorrect={false}` to preserve brand names and measurements)
- Character counter (small grey, right-aligned): "X / 40,000 characters" — turns red when text.length > 40,000
- Primary "Import Recipe" button — disabled when text.length < 50 OR text.length > 40,000
- Helper text: "Paste from anywhere — just grab the recipe from an email, text, or your favorite recipe pages."

**Original (pre-shipped):**
> - Header: "Import from Text"
> - Sub: "Paste a recipe — Kiwi parses it into ingredients and steps"
> - Large multiline text input (paste-friendly, accepts 100 to 40,000 characters)
> - Helper text: "Works with any text — copied from a website, an email, a note, a PDF, anywhere"
> - Primary button: "Import Recipe" (sub-text: "Kiwi parses the ingredients and steps from the text you pasted")

**Redline reasoning:**
- **Min-char dropped from 100 to 50.** Tupperware-lid spice mix testing during 6c-2 (Hans, May 12, 2026) confirmed legitimate recipes can be very short (just an ingredient list with implied prep). 100-char floor would have rejected those. Rate limiter (12/min) is the real abuse protection.
- **Helper text rewritten** for more natural / friendly tone, removing PDF (out-of-scope for MVP) and notes (covered by "anywhere").

#### 10.4b.2 Behavior on import

- Hit `POST /recipes/import-text` with the pasted text [SHIPPED 6c-3]
- Server passes text through `runAICall('import.reformat_for_kiwi', {rawText}, CanonicalRecipeSchema)` — same canonical-recipe AI pass as URL and Image imports
- Parser extracts: title, description, source attribution (if mentioned in text), prep/cook time (if mentioned), servings (if mentioned), ingredients (with quantities), recipe steps, cuisine type, difficulty
- Macros computed from ingredients × USDA-equivalent nutrition data (when available; estimated otherwise)
- Returns parsed meal record (in draft state, not yet saved)
- Client navigates **directly to meal-builder review screen** [REDLINE May 13, 2026 — shipped reality routes to /meal-builder, not a separate import-result screen. Same review UX as URL and Image imports.]
- **Cost: ~$0.05/import** (text-only, no vision). Free to user per §1.2.

#### 10.4b.3 Edge cases

**Not enough text to parse:** "Recipe text too short — paste at least 50 characters" [REDLINE May 13, 2026 — shipped copy; rejection happens client-side via disabled button, server validates as Zod 400 fallback]

**Text is way too long:** "Recipe text too long — maximum 40,000 characters" [REDLINE May 13, 2026 — shipped copy; rejection happens client-side, server validates as Zod 400 fallback]

**Text has no clear recipe structure:** AI returns `no_recipe_content` envelope. Surfaces as `TEXT_IMPORT_FAILURE_MESSAGE`: "Kiwi couldn't read this recipe text. Make sure you've pasted a complete recipe with ingredients and instructions."

**Text has ingredients but no steps (or vice versa):** Handled same way as §10.4.4 — AI generates suggested steps with a caveat. Caveat surfacing in UI deferred per D-WS6-046.

**Ambiguous quantities:** Same as URL Import — best-effort with user-editable notes.

**Duplicate detection:** Heuristic match on title + ingredient overlap; surface "You may have imported this recipe before" prompt with option to view existing or save as a new copy. [NOTE: duplicate-detection not implemented in MVP — defer.]
...
#### Anti-bot landscape (added May 11, 2026 post-6c-1 close)

**Reality check:** Recipe sites actively block server-side fetches via Cloudflare and other anti-bot infrastructure. As of May 2026, preflighted results from major US recipe sites:

| Site | Server-side fetch result |
|---|---|
| AllRecipes | Cloudflare JS challenge (blocked) |
| Serious Eats | HTTP 403 (blocked) |
| Simply Recipes | HTTP 403 (blocked) |
| Food52 | HTTP 429 (rate-limited / blocked) |
| Budget Bytes | HTTP 403 (blocked) |
| Smitten Kitchen | TCP connection reset (blocked) |
| Love & Lemons | HTTP 200 ✅ (works) |
| NYT Cooking | HTTP 308 redirect to paywall/recommendation (effectively blocked for non-subscribers; paywall for subscribers) |

**Implication for product positioning:**
- **URL Import is best-effort, not primary.** It works on personal blogs and a subset of sites that allow server-side access. It fails on most large publisher sites.
- **Image Import (§10.4) and Text Import (§10.4b) are the primary user paths** for recipe content from blocked sites. The UX should present all three as siblings (URL / Image / Text), not URL-first-with-fallbacks.
- **Cost implications:** Image Import costs $0.10-0.20/import (vision AI). Text Import costs ~$0.05/import (text-only AI). These are higher than URL Import (free when JSON-LD works, ~$0.01-0.02 when AI fallback fires), but both are free to the user per §1.2 (processing user-supplied content).

**Why we're not investing in workarounds for MVP:**
- **Headless browser per user** (Puppeteer/Playwright session replay) — multi-week build, ongoing maintenance, brittle, TOS risk. Rejected.
- **API partnerships** with publishers — months-to-years timeline, partner-dependent. Logged as D-WS6-035 for post-MVP roadmap.
- **Third-party scrape-as-a-service** (rotating proxies, Cloudflare-aware libraries) — added cost, vendor dependency. Revisit post-MVP if usage data justifies it.

**Telemetry to capture (pre-launch / WS9):** URL-import failure rate by domain. If a partnership opportunity emerges for a high-failure domain with high user-volume signal, prioritize then.

#### URL parsing (HTML → recipe)

**Two-tier approach (when the page is reachable):**
1. **Structured data first.** Many recipe sites embed JSON-LD or Microdata using schema.org's `Recipe` type. Extract this directly when present — high accuracy, zero AI cost.
...
**Anti-bot pre-check (added 6c-1):** Before any parsing, the fetch layer checks for Cloudflare challenge pages (≥2 of 6 markers detected → fail-fast with `cloudflare_challenge` error). HTTP 3xx redirects also fail-fast (NYT Cooking pattern: redirect to paywall/recommendation page would otherwise waste AI cost parsing the wrong content).

When the page IS reachable, this gets most recipes via structured data (free), and AI fallback (~$0.01/recipe) for the rest.

**Implementation note:** Use `recipe-ingredient-parser-v3` for ingredient parsing + cheerio for JSON-LD extraction. AI fallback uses the existing Anthropic API integration via `runAICall('import.reformat_for_kiwi', ...)`.
...
**[REDLINE PENDING WS7-CLOSE — WS7-7-A, June 11, 2026]** This "Regenerate or use existing list?" prompt model is superseded by the WS7-7 reconcile design (D-WS7-079). New behavior, to be written in at WS7-CLOSE: when a grocery list exists and the underlying plan has changed (tracked via `GroceryList.lastGeneratedFromPlanRevisionId` vs the plan's live `revisionId`), the list **auto-reconciles on read** — incrementally re-resolving only the delta (changed/added/removed meals' ingredients), preserving user-added Extras + recurring items, with a brief user-visible "updating your list to match plan changes" state. A manual full-regenerate escape hatch ("Something wrong? Have Kiwi regenerate the list") sits subtly at the bottom of the grocery screen. The blocking 409-on-existing behavior (current code) and this prompt-model paragraph both retire when the redline lands.
...
### 12.13 Retailer priority [REDLINED 6-CLOSE — Instacart primary; Whole Foods / Amazon Fresh deprioritized]

**Status as of WS6 close (May 18, 2026):** Original D1 / pre-PRD-lock framing assumed Whole Foods / Amazon Fresh via Playwright RPA could ship "without external dependency." Subsequent investigation (D-WS6-041 anti-bot landscape; broader retailer site reconnaissance during 6c-1) showed that path is unpromising — modern grocery sites use bot-detection layers that defeat RPA at scale, and affiliate-link flows don't carry full carts. **Instacart is now the priority target.** Whole Foods / Amazon Fresh paths are deprioritized indefinitely.

Backend grocery-list shapes (canonical ingredient IDs, normalized quantity/unit, ambiguous flagging) remain Instacart-friendly from day one (per WS6 architectural decision) so the eventual integration drops in cleanly when Instacart developer-program access lands.

For MVP:

| Retailer | Integration Type | Priority | Status |
|----------|------------------|----------|--------|
| Instacart | Direct API | 1 | **Pursue API / partnership access — primary target.** Backend data shapes already Instacart-compatible. |
| Whole Foods / Amazon Fresh | (was: Playwright RPA) | Deprioritized | Original RPA approach abandoned — anti-bot landscape makes it unviable. Re-evaluate only if Amazon ships a dev-friendly API. |
| Peapod / Stop & Shop | Playwright RPA | 3 | Build adapter post-MVP IF a viable browser-automation channel exists for that site (similar anti-bot caveat applies). |
| Walmart | Direct API preferred | Future | Post-MVP. API would be preferred over RPA. |
| Target | Direct API preferred | Future | Post-MVP. Same preference. |

**Vertical-slice retailer integration is deferred indefinitely** until Instacart access lands. Until then, "Order Online" remains stub UI. AI-agent / Computer Use approaches to retailer automation are novel territory worth investigating post-MVP but not in scope.
...
- ~~Action buttons at bottom: "Skip this phase" (for optional phases) | "Done with phase ✓" (advances to next)~~ *[superseded — see redline below]*

**[REDLINE July 4, 2026 — shipped reality (WS7-8b `0c5d60e`, BUG-020 / D-WS7-188; Hans-ruled July 4)]** Phase actions moved out of the phase card into the screen footer, and a write-free exit was added:

- The phase card keeps the phase indicator; there is **no action button in the card**. All actions live in the footer (the back arrow is retained).
- **Footer, all phases except the last:** **"Mark all complete"** (primary; marks the phase's remaining unchecked steps complete and advances — the renamed "Done with phase ✓") | **"Skip this Prep"** (write-free advance) | **"Save & Exit"** (write-free; exits to the plan-detail screen; no completion toast).
- **Last phase:** "Mark all complete" (= the "Finish prep ✓" behavior, celebratory toast) | "Save & Exit". No Skip.
- **Progress bar partial state:** a past phase with any unchecked step renders a distinct partial state (reduced-opacity sage), including skipped phases. Meals with unchecked steps remain not-prepped — the §13.4.4 derivation is unchanged — and Cook Mode prompts accordingly. This is the intended partial-prep semantics: exiting or skipping mid-prep never fabricates completion.

**[REDLINE ADDENDUM — applied July 12, 2026 at the WS7-8b close; shipped in Block A, `6227c6e` (BUG-024)]** **Auto-advance on full check.**

- When the user checks the **last remaining unchecked step** in a phase, the phase **auto-advances** — the user does not have to also press "Mark all complete." An intermediate toast confirms the phase completed. "Mark all complete" remains for the case where the user wants to force-complete a phase with steps still unchecked.
- **Double-advance guard.** The advance is edge-gated and latched (pure `prepAutoAdvance.ts`), verified against the container's real call interleavings: button-then-effect and effect-then-button both produce **exactly one** advance, never two; an empty-phase mount produces zero; re-completion re-advances.
- **⚠️ Known behavior delta (ruled acceptable, July 9, 2026):** the auto-advance effect fires on the **optimistic** write, so a *failed* "Mark all complete" server write will **advance and then revert**. This is ruled consistent with the existing per-checkbox path, which behaves the same way — the optimistic-then-revert pattern is uniform across the surface rather than special-cased here.
...
The Sequencer receives all step data from the meal's dishes (with phase types, estimated times, timing-sensitivity flags) and produces a single ordered sequence. It optimizes for:
1. **Nothing finishes materially early** (so the salad doesn't wilt, and the corn isn't cold, while the chicken is still cooking)
...
> **[REDLINED July 13, 2026 — WS7-8b BUG-018 close. Criterion 1 was "aligning finish times across dishes."]** **Under a single cook, identical finish times are often UNACHIEVABLE — and chasing them is the wrong objective.** *"Finish every dish at serve"* implicitly **assumes parallelism, but a human serializes active steps.** Only **unattended** windows permit overlap, so this is **single-resource scheduling with unattended gaps**. The enforceable invariant is therefore **"nothing finishes materially early / no cold food,"** NOT *"all finish times equal."*
>
> **⚠️ Criteria 1 and 2 genuinely conflict, and the ORDER OF THIS LIST IS LOAD-BEARING.** BUG-018 was an implementation optimizing **criterion 2 at the cost of criterion 1** — filling a passive window by pouring the salsa at minute 10 against a serve at minute 56, leaving cold accompaniments sitting **44–46 minutes**. **Criterion 1 wins.** A scheduler that minimizes elapsed time will reproduce the bug: the correct schedule is often **~3 minutes longer** precisely because it *refuses* to start a cold assemble early. *"A little late is fine; food is hot."*

**Implementation:**
- Sequencer runs server-side when Cook Mode launches for a multi-dish meal — **deterministically, in code** (`lib/cooking/cookingScheduler.ts`), per §13.5.5. **It is NOT an AI call.**
- Returns the ordered step sequence to the client
- Client renders this sequence as one Cook Mode flow
- **The output is a SORTED LIST, not a timeline.** The client walks it linearly, consuming order + cue. Steps also carry `startOffsetMinutes` (**serve-anchored, ≤0**), which is computed and tested but **not yet read by any consumer** — it exists for a future serve-time/notification feature. **No wall-clock time ever enters the Sequencer:** a consumer supplies the serve time T and derives `startAt = T − offset`, which keeps timezone/DST concerns structurally out of the scheduler.

[SCHEMA] **[REDLINED July 13, 2026 — WS7-8b BUG-018 B1/B2.]** The Sequencer uses **`phaseType` + `estimatedMinutes` + `isTimingSensitive`** to compute ordering. **`RecipeInstructionStep.parallelGroup` is RETIRED** — it was **NULL on 1959/1959 rows** (never written by any content path) and is now **write-rejected** (400) rather than silently dropped; the column is retained but unwritten. **`isTimingSensitive` is the discriminator that replaces it** and had the same structural defect until B1 (**true on 0 of 1959 rows** — `WizardStepSchema` could not express it). **The overlap predicate:** a step is unattended/overlappable if `phaseType ∈ {preheat, rest, hold}` **OR** (`phaseType == cook` **AND NOT** `isTimingSensitive`). **Duration ≠ attention** — a 30-min braise is hands-free; a 3-min sear is not. **The serve anchor is the longest-total-duration dish, NOT `roleLabel`** (`MealDishLink.roleLabel` is `@default(main)`, so a 3-dish meal with nothing explicitly labeled yields *three* mains, not one).
...
> **[VERIFIED July 13, 2026 — WS7-8b BUG-018 B2.] This clause is now TRUE. It was not true before.** The Sequencer had been implemented as a **Sonnet call** (`sequencer.step_ordering`) — rate-limited 12/min, one `LLMCallLog` row **per Cook Mode launch**, uncached. It is now a pure code path (`lib/cooking/cookingScheduler.ts`): **`usedAI=false`, ~290ms, `LLMCallLog` delta = 0.** **BUG-018 was therefore FIX-TO-CONFORM, not a spec change — this section already said *deterministic* and already priced it *free* on that basis; the implementation simply did not conform.**
>
> **⚠️ This clause is load-bearing pricing, not description.** Any future change that reintroduces an AI call into the Sequencer **breaks the free-tier promise made here** and must redline this section first.
...
#### 13.5.7 Cook completion [REDLINED July 13, 2026 — owner: WS7-11. NOT YET BUILT.]

> **⚠️ STATUS: NOT BUILT.** Today, Cook Mode's step list simply **ends** — there is no completion screen. The user backs out with the arrow, which means **cooking a meal to completion and abandoning it halfway are, to the app, the identical gesture.** This is owned by **WS7-11 — "Favorites, Saves, Ratings"** (D-WS7-202), sequenced **after the WS9 restyle** so this net-new screen is built once, in the new visual language.

After advancing through final step:
- **Confirmation screen (copy APPROVED by Hans, July 13, 2026 — replaces the original *"Done! How was [meal name]?"*):**
  - Header: **"All done — enjoy a great meal by Chez [Name]!"**
  - Sub-line: **"How was it? Your rating tunes future plans."**
  - `[Name]` = the user's `firstName`, which is **non-nullable** (`schema.prisma:173`), so `Chez [Name]` **always resolves**. *"Chez You"* is a defensive fallback for the empty-string edge case only — **not the common path.**
  - **Rationale for the sub-line (preserve this — it is the reasoning, not decoration):** the rejected draft was *"Rate your meal to help Kiwi improve."* That framing **asks the user to perform unpaid labor for us** — it presents the rating as a favor to the company. The shipped line frames the ask as **self-interested** (rate it → Kiwi gets better at recommending *to you*), which is **true**, and materially better for completion rate.
- Optional rating: 1-5 stars (per §13.7 — feedback loop). **Saves on tap.**
- **ONE button: "Done" → routes to Meal Detail.**
  > **[REDLINED — this section previously specified *"Two buttons: 'Back to Plan' | 'Done'"*, and §13.10.1 independently specified a *different* pair, *"Save updates" | "Skip"*. Built verbatim, the screen would carry FOUR buttons and TWO competing exits. The spec was failing to reconcile itself.]**
  > **The ruling (Hans, July 13, 2026):** one button. Stars save on tap; time/difficulty edits (§13.10.1) stage locally and **commit on exit**. **Touching nothing writes nothing — that IS "Skip,"** so no Skip button is needed; the hungry user leaves in one tap. **"Back to Plan" is dropped:** Meal Detail already has a nav stack beneath it (the plan is one tap away), and Cook Mode **also launches from the Library with no `planId` at all** — the button would be dead half the time.
  > **The back arrow MUST behave identically to "Done"** (commit staged edits, then leave). Discarding on back would build a silent data-loss trap: the user corrects 30 min → 50 min, hits back, and the correction evaporates.
- **Activity logged:** `cook_mode_completed` (per §13.7) with meal_id, plan_id, duration.
  > **[REDLINED — this section previously said `meal_cooked`, which does not exist in §13.7's event list.]** The canonical event is **`cook_mode_completed`**. §13.10 adds **`meal_attributes_refined`**; a rating emits **`meal_rated`**.
- **⚠️ THIS SCREEN IS THE WRITE POINT FOR `timesCooked` / `lastCookedAt`** — see §13.10.4 (new).
...
- ~~Parallel grouping: identify steps that can happen concurrently with passive steps from other dishes~~ **[REDLINED July 13, 2026 — WS7-8b BUG-018 B1.]** **This described the retired `parallelGroup` field and is no longer a translation-time concern.** Concurrency is **derived at schedule time, in code**, from `phaseType` + `isTimingSensitive` (see §13.5.4) — not tagged into the steps by the translation pass. **What translation DOES still owe the Sequencer is the timing-sensitivity flag** (the bullet above): mark a step timing-sensitive when it demands continuous attention (searing, sautéing, frequent turning) and NOT when it is hands-off (baking, roasting, braising, boiling, simmering). **That single flag is what makes overlap safe or unsafe** — get it wrong and the scheduler will either stack prep onto a step that needs watching, or serialize a 30-minute braise it could have cooked underneath.
...
- ~~Two buttons: "Save updates" | "Skip"~~ **[REDLINED July 13, 2026 — see §13.5.7.]** **The card has NO buttons of its own.** It is inline on the completion screen, which carries **one** button ("Done"). Edits **stage locally and commit on exit**; **touching nothing writes nothing — that IS "Skip."** *(This section and §13.5.7 each independently specified "two buttons" and named **different pairs** — built verbatim that is four buttons and two competing exits. §13.5.7 carries the reconciliation.)*

If user skips, no changes saved. If user updates, the edits persist on the user's saved Meal record.

> **⚠️ WHY THIS CARD MATTERS MORE THAN IT LOOKS (do not cut it for scope):** this is the **only place in the entire product** where a user can correct a bad time estimate. And `estimatedMinutes` is **load-bearing input to the Cooking Sequencer** (§13.5.4) — BUG-018 was fundamentally *about* time estimates. This card is the feedback loop that keeps that number honest, per-user, forever. **It also never gets retrofitted:** once cooking ships without it, adding it later means re-opening Cook Mode.
...
> **⚠️ FORK-ON-CALIBRATE IS LOAD-BEARING — this is the one path in WS7-11 that can corrupt SHARED data.** A rating or a time correction is a **personal** value. **Never write one to a `userId: null` catalog row** — that would publish one user's opinion to every user. Rating/calibrating an unsaved curated meal **must FORK it first**. **The machinery already exists and must be REUSED, not reinvented:** D-WS7-139 (WS7-7-A B5) built fork-on-acquire, gated on not-already-owned, and wired it at add-to-plan / use-template / meal-swap. WS7-11 adds calibrate/rate as a fourth acquisition trigger.
...
[SCHEMA] ~~No new fields required.~~ **⚠️ [REDLINED July 13, 2026 — THIS NOTE WAS WRONG, and it was WS7-11-blocking.]** It considered only `estimatedTimeMinutes` and `difficulty` (which do exist on `Meal`) — **it forgot the stars.** There is **nowhere on `Meal` to store a 1–5 rating today**, and nowhere to store a favorite. **WS7-11 requires a migration.** The user's existing `estimatedTimeMinutes` / `difficulty` fields are updated in place post-cook, as originally stated; the **rating**, the **favorite flag**, and the **cook-tracking fields** (§13.10.4) are net-new. Activity event `meal_attributes_refined` (new) tracks when refinement happens for analytics.

#### 13.10.4 Favorites, ratings, and cook-tracking [NEW — added July 13, 2026. Owner: WS7-11. NOT YET BUILT.]

**Three concepts that are commonly conflated, and must not be.** The product currently has controls for only the first.

| Concept | Control | Question it answers | Status |
|---|---|---|---|
| **Saved** | **Heart** (on public meal/dish cards, per §9.5) | *"Is this in my library?"* — an **acquisition** gesture | ✅ Shipped. **NOT a rating.** |
| **Rated** | **1–5 stars** | *"Was it good?"* — retrospective **quality** | ❌ Net-new (WS7-11) |
| **Favorite** | (toggle, TBD) | *"Is this a staple? Put it in front of me when I'm planning."* — forward-looking **intent** | ❌ Net-new (WS7-11) |

> **⚠️ STARS CANNOT SUBSUME FAVORITES. Quality and reach-for-it-ness are UNCORRELATED.** A **5-star** meal can be a once-a-year showpiece — excellent, and not a staple. A **3-star** meal the kids will actually eat on a Tuesday is **exactly** a favorite. The favorite-ness comes from *easy* and *the kids will eat it*, not from the score. **Do not collapse these into one field.**
>
> **⚠️ NOR DO STARS SUBSUME THE HEART.** The heart is the **save-to-library** control (§9.5) — deleting it in favor of stars would delete the save button. *(Note §326 and §2048 contradict each other on whether a `UserFavorite` record exists; one is stale. **Verify against running code, not this document,** at WS7-11 Phase 0.)*

**Rating placement — TWO mount points, not one.** The rating is a **property of the Meal**, not a widget on the completion screen. It renders and is editable on:
1. The **cook-completion screen** (§13.5.7) — the first and most likely moment to rate.
2. **Meal Detail** — so a user can rate later: *"oh, I cooked this last week and it was great"* / *"that one, I didn't like much."*

**Cook-tracking — `timesCooked` / `lastCookedAt`.**
- **⚠️ Nothing in the product currently knows a meal was cooked.** `Dish.timesCooked` and `Dish.lastUsedAt` are **dead columns** — a DB probe returned **0/337 and 0/337**; nothing has ever written to them. That is precisely why the **`last_cooked` sort option is greyed out** in the app: the UI offers the sort, the data does not exist. (§13.7 specs `cook_mode_started` and `cook_mode_completed`; **neither is emitted.**)
- **`mealUseCount` is NOT a cook count.** It is a **plan-link count** (how often a meal was put *into a plan*) and backs the "Most used" sort. **Planning a meal and cooking it are different events** — the entire prep/cook arc is evidence that users do not cook everything they plan.
- **RULING: the counter increments on DONE, not on Cook-tap.** Tapping **Cook Now** expresses *intent* — exactly the intent abandoned when the kid melts down at step 3. Counting on **launch** inflates the number with meals nobody ate and **corrupts the one thing the number is for** (*"show me my staples"*): a meal you started and bailed on is the **opposite** of a staple. **The completion screen is the write point.**
- **Naming reconcile owed at WS7-11 Phase 0:** the column is `lastUsedAt`, but the semantics WS7-11 gives it are *lastCooked*. Rule the name before writing to it.

**OPEN — must be ruled before WS7-11 commissions: what do favorites actually DO?** A favorite toggle that visibly does nothing is **worse than no toggle** (users mark favorites, nothing changes, trust erodes). Candidate surfacings: a **Favorites filter/sort on My Meals** · **wizard bias** (weight plan generation toward favorites — note this is a **prompt-contract change**, not a UI change) · **add-to-plan pickers** (favorites pinned to top). See **D-WS7-202**.

**OPEN — the sharing collision (D-WS7-203).** Sharing-lite (roadmap row 7) specifies **like / dislike** (thumbs) on public meals. **The real collision is stars-vs-THUMBS** — two quality signals on **different scales**, meeting on the Cookbook browse surface. The thumbs were almost certainly specified **before stars existed**, so this is reconcilable rather than locked. **WS7-11 rules it; sharing consumes the ruling.** ⚠️ **Before ruling, check what the *dislike* actually feeds** — if it drives *suggestion suppression* rather than display, it is **not** redundant with a star and cannot simply be deleted.
...
- `Subscription` row created with `status: 'trialing'`, `planCode: 'premium_monthly'`, `trialEndsAt: signup + 14 days` ⚠️ **REDLINED August 11, 2026 (D-WS9-138): was 30 days.** ⚠️ **`planCode` DIVERGES FROM THE CODE — see D-WS9-143 in the deferred decisions log** (open question, owned by WS10; the substance lives there, not here).
- No credit card required
- ⛔ **STRUCK August 11, 2026 (D-WS9-138). THIS MECHANISM NEVER EXISTED.** There is no `DEFAULT_TRIAL_DAYS` env var and no constant — the server carried a bare `30 * 24 * 60 * 60 * 1000` literal at `auth.ts:126`. Block 2a replaced it with a named constant set to **14**, cross-referenced to the mobile constant by a KEEP-IN-SYNC comment. ⚠️ **There is still no env override and no admin control.** If either is wanted, it is unbuilt work — not a documented feature.
...
- Plan length default *[REDLINED 2026-05-21 / WS7-2-F: lifted from FUTURE. Settable in onboarding step 2 + profile preferences screen. Default 5, range 1-7.]*
...
## Change Log (v1.1 working onward)

### 2026-05-11 — 6c-1 sub-workstream close (Reformat-for-Kiwi + URL Import + scale refactor)
**Sections affected:** §10.2, §10.3.1, §10.3.4, §10.4b (new), §10.9
**Change:** Recipe Import surface redesigned to reflect anti-bot landscape reality. Preflight during 6c-1 fix-2 found that 5-of-6 major recipe sites (AllRecipes, Serious Eats, Simply Recipes, Food52, Budget Bytes, Smitten Kitchen) block server-side fetches via Cloudflare or other anti-bot infrastructure; only Love & Lemons worked. NYT Cooking returns HTTP 308 redirects to paywall/recommendation pages. URL Import is therefore best-effort, not primary. Image Import and a new Text Import sub-method become primary paths for major-site content.

Specific changes:
- §10.2: Added Text Import as 3rd sibling in all 4 entry-point lists. Added note that sibling ordering is by convention; URL is no longer the recommended default given anti-bot landscape.
- §10.3.1: Replaced misleading "Works with AllRecipes, Food Network, NYT Cooking, Serious Eats" helper text with "Works with many recipe sites. If a URL doesn't work, try Import from Image or Import from Text instead." Added explanatory note about anti-bot reality.
- §10.3.4: Replaced "Try a different link, or [Create Manually]" error UX with three buttons: Import from Image (primary), Import from Text (primary), Create Manually (secondary).
- §10.4b: New section — Text Import sibling import method. Paste recipe text, 100-40,000 char range, AI parses via same `import.reformat_for_kiwi` helper used by URL and Image imports. Cost ~$0.05/import, free to user per §1.2.
- §10.9: New "Anti-bot landscape" subsection at the top documenting blocked-site landscape, product positioning implication, and why we're not investing in headless-browser / partnership / third-party-scrape workarounds for MVP (logged as D-WS6-035, D-WS6-041 for post-MVP roadmap).

**Decided by:** Hans (product call during 6c-1 close, May 11, 2026). Per kiwi_deferred_decisions_log.md D-WS6-041 + kiwi_ws6_plan.md §4 "From 6c-1 close" locked decisions.

---

Initial state: identical to v1.0 (frozen April 30, 2026). No deltas yet.
...
**Decided by:** Hans (or whoever)
```

---

### 2026-07-13 — WS7-8b CLOSE → WS7-8 CLOSE (BUG-018 deterministic Sequencer; the post-cook loop becomes WS7-11)
**Sections affected:** §13.5.4, §13.5.5, §13.5.7, §13.6, §13.10.1, §13.10.2, §13.10.3, **§13.10.4 (NEW)**

**Driver:** BUG-018 ✅ FIXED (`d561f65` + `141c9b9`, pushed, device-verified) — *"corn was boiled during a 30-min grill and came out well before the meat finished."*

**The core finding — this was FIX-TO-CONFORM, not a spec change.** §13.5.5 **[LOCKED]** already stated the Sequencer *"runs deterministically on existing step data"* — **and priced it FREE on that basis.** §13.5.4 **[LOCKED]** already made finish-time alignment its first criterion. **The implementation did not conform:** it was a **Sonnet call** (`sequencer.step_ordering`), rate-limited 12/min, one `LLMCallLog` row **per Cook Mode launch**, uncached. The PRD had said *compute* all along.

**⚠️ And the prompt was ALREADY CORRECT.** `aiPrompts.ts:1348-1352` already instructed *"work backwards from the longest dish,"* with a worked corn/roast example at `:1391-1409`. **The AI had the right instructions, the right data, and still got it wrong — no prompt edit could have fixed this.** Captured live on a real 3-dish meal, AI and deterministic scheduler run side-by-side *before the working path was deleted*: the **AI** poured the salsa at **minute 10** and portioned sour cream at **11–12** against a **serve at 56** — cold accompaniments **sitting 44–46 minutes** — offering the confident, plausible cue *"Pour the salsa now while your hands are free."* The **deterministic** scheduler placed them at **−1** and **−3/−6**. Nothing cold sits.

**It was optimizing criterion 2 (filling passive windows) at the cost of criterion 1 (finish-time alignment). A weighting defect, not a prompt defect.**

**The reframe that drove the §13.5.4 redline:** *"finish every dish at serve"* **implicitly assumes parallelism, but a human serializes active steps.** Only **unattended** windows permit overlap → this is **single-resource scheduling with unattended gaps**. **Consequence: under a single cook, identical finish times are often UNACHIEVABLE.** The enforceable invariant is **"nothing finishes materially early / no cold food,"** NOT *"all finish times equal."*

**Specific changes:**
- **§13.5.4** — criterion 1 rewritten to *"nothing finishes materially early"*; the **criteria ORDER flagged load-bearing** (they genuinely conflict, and BUG-018 is what happens when 2 beats 1); implementation note now states the Sequencer is **deterministic code, not an AI call**, that its output is a **sorted list, not a timeline**, and that **no wall-clock time ever enters it** (steps carry serve-anchored `startOffsetMinutes ≤ 0`; a consumer supplies T and derives `startAt = T − offset`, keeping timezone/DST bugs structurally out). `[SCHEMA]` note redlined: **`parallelGroup` RETIRED** (**NULL on 1959/1959** — never written by any content path; now **write-rejected 400** rather than silently dropped) → the Sequencer reads **`phaseType` + `estimatedMinutes` + `isTimingSensitive`**. Documents the **overlap predicate** (`{preheat, rest, hold}` OR `cook && !isTimingSensitive`), that **duration ≠ attention** (a 30-min braise is hands-free; a 3-min sear is not), and that the **serve anchor is the longest-total-duration dish, NOT `roleLabel`** (`@default(main)` → a 3-dish meal with nothing labeled yields *three* mains).
- **§13.5.5** — the deterministic/free clause **verified TRUE** (`usedAI=false`, ~290ms, `LLMCallLog` delta = 0); flagged as **load-bearing pricing, not description** — any future AI call here breaks the free-tier promise and must redline this section first.
- **§13.6** — the "parallel grouping" translation bullet described the retired field; **concurrency is now derived at schedule time from `isTimingSensitive`**, not tagged at translation time. What translation still owes the Sequencer is the **timing-sensitivity flag**, and that single flag is what makes overlap safe or unsafe.
- **§13.5.7** — **NOT BUILT** (flagged): Cook Mode's step list simply *ends* today, so **cooking to completion and abandoning halfway are the identical gesture**. Header copy **APPROVED by Hans**; **the two-buttons contradiction with §13.10.1 RESOLVED to ONE button** ("Done" → Meal Detail) — each section independently specified *"two buttons"* and named **different pairs**, which built verbatim is **four buttons and two competing exits**; **the back arrow must commit, not discard** (else a corrected time estimate silently evaporates); event name `meal_cooked` **corrected to `cook_mode_completed`** (`meal_cooked` never existed in §13.7's list).
- **§13.10.1 / §13.10.2** — calibration card has **no buttons of its own** (staged edits commit on exit; **touching nothing writes nothing — that IS "Skip"**); **fork-on-calibrate flagged as the one path that can corrupt SHARED catalog data** — a rating is a *personal* value and must **never** be written to a `userId: null` row; **reuse D-WS7-139's existing fork-on-acquire**, don't reinvent it. Also records *why the card matters*: it is the **only place in the product** where a user can correct a bad time estimate, and `estimatedMinutes` is **load-bearing input to the Sequencer**.
- **§13.10.3 `[SCHEMA]` — THE NOTE WAS WRONG, and it was WS7-11-blocking.** It claimed *"No new fields required"* because it counted only `estimatedTimeMinutes`/`difficulty` — **it forgot the stars.** There is **nowhere on `Meal` to store a 1–5 rating today**. **WS7-11 requires a migration.**
- **§13.10.4 (NEW) — Favorites, ratings, and cook-tracking.** **Three concepts, commonly conflated, that must not be:** **Saved** (the **heart** = save-to-library per §9.5 — **NOT a rating**) · **Rated** (**stars** = retrospective quality) · **Favorite** (**forward-looking intent** — "this is a staple, put it in front of me when planning"). **⚠️ Stars cannot subsume favorites: quality and reach-for-it-ness are UNCORRELATED** — a 5-star meal can be a once-a-year showpiece, while a 3-star meal the kids will eat on a Tuesday is *exactly* a favorite. **Nor do stars subsume the heart** (that would delete the save button). Documents that the **rating has TWO mount points** (completion screen *and* Meal Detail, so a user can rate a week later), that **`timesCooked`/`lastUsedAt` are dead columns** (**DB-probed 0/337** — *nothing in the product currently knows a meal was cooked*, and this is why the `last_cooked` sort is greyed out), that **`mealUseCount` is a plan-link count, not a cook count** (planning ≠ cooking), and **rules that the counter increments on DONE, not on Cook-tap** (Cook-tap is *intent*; counting it inflates the number with meals nobody ate and corrupts the one thing it's for — a meal you bailed on is the **opposite** of a staple).

**Ownership:** all §13 post-cook items belong to the **new WS7-11 workstream — "Favorites, Saves, Ratings"** (D-WS7-202), **sequenced AFTER the WS9 restyle** — the completion screen is **net-new UI**, and building it before the restyle means **designing it twice**.

**Still OPEN (must be ruled before WS7-11 commissions):** **(1) What do favorites actually DO?** (My Meals filter/sort · wizard bias — a **prompt-contract** change · add-to-plan pinning). *A favorite toggle that visibly does nothing is worse than no toggle.* **(2) The sharing collision (D-WS7-203)** — sharing-lite specifies **thumbs** (like/dislike); WS7-11 introduces **stars**. **The real collision is stars-vs-thumbs**, and they meet on the Cookbook browse surface. The thumbs were almost certainly specced *before* stars existed, so this is reconcilable. **⚠️ Check what the *dislike* actually feeds first** — if it drives suggestion *suppression* rather than display, it is not redundant with a star.

**Cross-ref:** BUG-018 (bug log), D-WS7-202 / -203 / -204 / -205 / -206, D-WS7-139 (fork-on-acquire), D-WS7-111 (the dead columns), D-WS7-164 (the regression this pins), roadmap rows 2 / 6a / 7.
