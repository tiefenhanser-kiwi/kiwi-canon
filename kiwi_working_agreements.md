...
- **§24.6 — Fresh-chat priming SHOULD load:** the project-instructions stub (auto — four lines), **then this file in full** (§21.1), `kiwi_navigation.md`, **the current-position block at the top of `kiwi_remediation_progress.md`, above `## 0` (the single source of position — §A)**, the most recent complete-handoff (**that is `kiwi_ws7_complete_handoff.md`; WS5's and WS6's moved to the mirror September 17, 2026 — §24.11**), and open deferrals filtered to the active owner. ~20–30KB total. ⚠️ **That block is the FIRST substantive read after navigation, not an optional extra** — it is the only place current block and HEAD exist. **Counters are NOT there; take them from each log** (§A(1)).
...
- **§24.11 — ⚠️ EIGHT FROZEN DOCS LEFT PROJECT KNOWLEDGE FOR THE MIRROR, September 17, 2026 — MOVED, NOT DELETED, AND NOT ARCHIVES.** §24.9's levers compress or split a doc; §24.10 keeps the pieces greppable. **This is the third and cleanest lever, and it is §16.3.1's division of labour applied literally: a doc that no fresh chat auto-loads does not belong in the bounded store.** Moved, each one whole and uncompressed, to `C:\Cooking App\kiwi-canon\`: **`kiwi_prd_v1.0_locked.md` · `kiwi_ws5_complete_handoff.md` · `kiwi_ws6_complete_handoff.md` · `kiwi_plan_generation_arc_scope.md` · `kiwi_ux_redesign_handoff.md` · `kiwi_screens_mockup.html` · `kiwi_prep_cook_mockup.html` · `kiwi_acceleration_review_2026-09-07.md`.** ✅ **Freed 152K tokens in one pass (1.661 MB → 1.509 MB) — more than both September 17 archive splits together, and it cost no compression and lost no content.**
    - ✅ **WHAT STAYED, AND WHY THE LINE IS WHERE IT IS: `kiwi_ws7_complete_handoff.md` and `kiwi_ws7_plan.md` STAY — WS7 is NOT frozen** (WS7-9 · WS7-10 · WS7-11 · WS7-CLOSE are all open, and roadmap row 5 commissions from that plan). **§24.6 names the most recent complete-handoff as a priming read and that is WS7's.** Chat-Claude proposed moving both and withdrew it on reading the roadmap — **the test is FROZEN, not OLD.**
    - 🔴 **THE VERIFICATION THIS NEEDED, AND IT IS A NEW HAZARD WORTH THE SECTION ON ITS OWN: `project_read` HANDS BACK A DOC UNDER ROUGHLY 100 KB AS INLINE TEXT, AND A LARGER ONE AS A LOCAL FILE PATH.** For the inline ones **there is no mechanical copy path at all** — the body has to be reproduced from tool output, so a mirror copy of them is a TRANSCRIPTION, and `cmp` against that copy proves only that the device matches the local scratch file, **never that either matches canon.** ⚠️ **Deleting the canon original on that evidence is how a silent corruption becomes the only surviving version** (§16.3.1's whole argument for git history).
    - ✅ **WHAT WAS ACTUALLY DONE, AND IT IS THE STANDARD FOR ANY FUTURE MOVE: the one file-backed doc (the 308 KB PRD) was verified end-to-end by hash, canon file → mirror. The seven inline ones were transcribed TWICE, by two independent agents that were forbidden to look at each other's output, and BOTH produced byte-identical SHA-256 and identical line counts across all seven.** **Two independent transcriptions agreeing is the oracle that a single one cannot be.** ⚠️ **Nothing was deleted until every hash matched.**
    - ⚠️ **AND THE ORDER IS NOT OPTIONAL: copy → verify by hash → THEN delete. Never delete first, and never treat the bridge's success response or a `cmp` against your own scratch copy as the verification.**
